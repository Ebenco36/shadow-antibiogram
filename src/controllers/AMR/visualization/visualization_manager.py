...
class VisualizationManager:
    ...
    def plot_umap_tsne_for_best_configs(self, output_dir: Path):
        ...
        For each BEST (genus, material, metric) configuration:
          ...
          1. Rebuild the similarity matrix (antibiotic x antibiotic).
          2. Rebuild the graph via GraphBuilder(tau) and LouvainClusterer(gamma)
             to get communities (partition) – used as hover info.
          3. Run t-SNE and UMAP on the similarity rows.
          4. Create THREE colourings:
               - by WHO class
               - by fine classification
               - by broad group

        Files written under `output_dir` (e.g. results/embeddings):

          tsne_<genus>_<material>_<metric>_broad.html/png/pdf
          tsne_<genus>_<material>_<metric>_fine.html/png/pdf
          tsne_<genus>_<material>_<metric>_who.html/png/pdf

          umap_<genus>_<material>_<metric>_broad.html/png/pdf
          umap_<genus>_<material>_<metric>_fine.html/png/pdf
          umap_<genus>_<material>_<metric>_who.html/png/pdf
        """
        agg_df = self.results.aggregated_results
        if agg_df is None or agg_df.empty:
            return
        ...
        # Label maps for WHO / fine / broad
        label_manager = LabelManager(self.config)
        label_maps = label_manager.get_label_maps()
        broad_map = label_maps.get("broad", {})  # dict abx -> broad class
        fine_map = label_maps.get("fine", {})    # dict abx -> fine class
        who_map = label_maps.get("who", {})      # dict abx -> WHO group
        ...
        for _, row in best_df.iterrows():
            ...
            # Subset the table to this (genus, material)
            df_subset, antibiotic_columns = self._prepare_subset_for_network(
                df_raw,
                genus,
                material,
            ...
            # --- Similarity matrix (same as for networks) ---
            sim_engine = SimilarityEngine(df_subset, antibiotic_columns)
            similarity_matrix = sim_engine.compute(metric)
            ...
            # --- Graph + Louvain communities (hover info) ---
            graph_builder = GraphBuilder(tau)
            G = graph_builder.build_graph(similarity_matrix)
            ...
            communities = communities.reindex(similarity_matrix.index)

            # --- Label series aligned with the similarity matrix index ---
            index = similarity_matrix.index

            broad_labels = pd.Series(
                [broad_map.get(a, "Unknown") for a in index],
                index=index,
                name="broad_class",
            )
            fine_labels = pd.Series(
                [fine_map.get(a, "Unknown") for a in index],
                index=index,
                name="fine_class",
            )
            who_labels = pd.Series(
                [who_map.get(a, "Unknown") for a in index],
                index=index,
                name="who_group",
            )

            # Common hover data (antibiotic + all labels + community)
            hover_df = pd.DataFrame(index=index)
            hover_df["antibiotic"] = index
            hover_df["community"] = communities
            hover_df["broad_class"] = broad_labels
            hover_df["fine_class"] = fine_labels
            hover_df["who_group"] = who_labels

            # --- Compute embeddings ---
            # Adjust perplexity for small n
            max_perplexity = max(1.0, (n_items - 1) / 3.0)
            local_perplexity = min(
                self.embedding_projector.config.tsne_perplexity, max_perplexity
            )

            try:
                coords_tsne = self.embedding_projector.compute_tsne(
                    similarity_matrix,
                    perplexity=local_perplexity,
                )
            except Exception as e:
                print(
                    f"[EmbeddingProjector] t-SNE failed for "
                    f"{genus}/{material}/{metric} with n={n_items}: {e}"
                )
                coords_tsne = None

            try:
                # Use rows as feature vectors (not precomputed distances)
                coords_umap = self.embedding_projector.compute_umap(
                    similarity_matrix,
                    precomputed_distance=False,
                )
            except Exception as e:
                print(
                    f"[EmbeddingProjector] UMAP failed for "
                    f"{genus}/{material}/{metric} with n={n_items}: {e}"
                )
                coords_umap = None
            ...
            safe_mat = (material or "ALL").replace(" ", "_")
            safe_metric = metric.replace(" ", "_")
            base = f"{safe_genus}_{safe_mat}_{safe_metric}"

            # --------------------- t-SNE plots --------------------- #
            if coords_tsne is not None:
                # Colour by BROAD group
                fig_tsne_broad = self.embedding_projector.plot_embedding(
                    coords_tsne,
                    labels=broad_labels,
                    label_name="Broad class",
                    method="TSNE",
                    title=f"t-SNE – {genus} / {material} / {metric} (Broad class)",
                    hover_data=hover_df,
                )
                self.embedding_projector.save_figure(
                    fig_tsne_broad,
                    output_dir / f"tsne_{base}_broad",
                    save_png=True,
                    save_pdf=True,
                )
                ...
                fig_tsne_who = self.embedding_projector.plot_embedding(
                    coords_tsne,
                    labels=who_labels,
                    label_name="WHO group",
                    method="TSNE",
                    title=f"t-SNE – {genus} / {material} / {metric} (WHO group)",
                    hover_data=hover_df,
                )
                self.embedding_projector.save_figure(
                    fig_tsne_who,
                    output_dir / f"tsne_{base}_who",
                    save_png=True,
                    save_pdf=True,
                )
            ...
            if coords_umap is not None:
                # Colour by BROAD group
                fig_umap_broad = self.embedding_projector.plot_embedding(
                    coords_umap,
                    labels=broad_labels,
                    label_name="Broad class",
                    method="UMAP",
                    title=f"UMAP – {genus} / {material} / {metric} (Broad class)",
                    hover_data=hover_df,
                )
                self.embedding_projector.save_figure(
                    fig_umap_broad,
                    output_dir / f"umap_{base}_broad",
                    save_png=True,
                    save_pdf=True,
                )
                ...
                fig_umap_who = self.embedding_projector.plot_embedding(
                    ...
                    label_name="WHO group",
                    method="UMAP",
                    title=f"UMAP – {genus} / {material} / {metric} (WHO group)",
                    hover_data=hover_df,
                )
                self.embedding_projector.save_figure(
                    fig_umap_who,
                    output_dir / f"umap_{base}_who",
                    save_png=True,
                    save_pdf=True,
                )
